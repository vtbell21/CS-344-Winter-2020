...
fn main() {
    ...
    // Create two threads and join the two threads
    ...
    // Call intermediate_sums on the result
    ...
    for i in 0..num_partitions {
        let vec_clone = vec_parts.clone();

        let handle = thread::spawn(move || map_data(&vec_clone[i]));
...
fn partition_data(num_partitions: usize, v: &Vec<usize>) -> Vec<Vec<usize>>{
    ...
    let mut xs: Vec<Vec<usize>> = Vec::new(); // Create a Vec of Vecs 
    let size = v.len() / num_partitions; // Calculate the size of the partition
    let mut mod_val = v.len() % num_partitions; // Calculate modulus value
    ...
    for i in 0..num_partitions{
        ...
        xs.push(x);
        if mod_val > 0 { // This loop will occur if num_elements is not a multiple of num_partitions
            xs[i].push(v[end]);
            mod_val -= 1;
            beg = beg + size + 1;
            end = end + size + 1;    
