...
int main(int argc, char* argv[]) {
	...
	while (1) {
		...
		int keyToInt = 0;
		int encryptToInt = 0;
		char* enc = calloc((140000) + 1, sizeof(char));
		...
		for (int i = 0; i < NewBufferLength; i++) {
			...
			encryptToInt = textToInt + keyToInt;
			encryptToInt = encryptToInt % 27;

			enc[i] = encryptToInt + 65;

			// If textToInt is out of ascii range of alphabet, Then set it to a space
			if (enc[i] == 91) {
				enc[i] = 32;
			}
		}
		...
		charsRead = send(connectionSocket, enc, strlen(enc), 0);
			if (charsRead < 0) {
